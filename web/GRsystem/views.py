...
def admin_login(request):
    return render(request,"GRsystem/admin_login.html")
...
def admin_dashboard(request):
    ...
    return render(request, "GRsystem/admin_dashboard.html", context)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and profile_form.is_valid():
            try:
                ...
                profile.type_user = request.POST.get('type_user', 'student')
                profile.save()
                
                messages.add_message(request, messages.SUCCESS, 'Registration successful! You can now login.')
                return redirect('/login/')
                
            except Exception as e:
                messages.add_message(request, messages.ERROR, f'Registration failed: {str(e)}')
        else:
            # Display form errors
...
def dashboard(request):
    ...
    pending_complaints = user_complaints.filter(status=3).count()
    in_progress_complaints = user_complaints.filter(status=2).count()
    solved_complaints = user_complaints.filter(status=1).count()
    
    # Get recent complaints (last 5)
    recent_complaints = user_complaints.order_by('-Time')[:5]
...
def complaints(request):
    ...
    if request.method == 'POST':
        ...
        if complaint_form.is_valid():
               ...
               print(mail)
               try:
                   send_mail('Hi Complaint has been Received', 'Thank you for letting us know of your concern, Have a Cookie while we explore into this matter.  Dont Reply to this mail', 'testerpython13@gmail.com', [mail],fail_silently=True)
               except Exception as e:
